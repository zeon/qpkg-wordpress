...
#		Wordpress v3.0.1 + NextGen Gallery v1.6.2 preinstalled (under wp-content/plugins/nextgen-gallery)
#
#	HISTORY:
...
post_update()
{
	$CMD_CP -af "${TMP_DIR}/wp-config.php" ${QPKG_CONFIG_PATH}
}
	...
	if [ -f "${QPKG_SOURCE_DIR}/${QPKG_SOURCE_FILE}" ]; then
		...
		if [ $? = 0 ]; then		
			...
			if [ ${UPDATE_FLAG} -eq 1 ]; then
				...
				$CMD_CHMOD 777 -R "${QPKG_DIR}/wp-content"
			fi
